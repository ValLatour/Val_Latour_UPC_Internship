...
public class SC_DCB_I_Model implements Model {
    GRBEnv env;
    GRBModel model;
    GRBVar[][][] x;
    GRBVar[] z;
    ...
    public String model_name(){return "SC_DCB_I";}

    public SC_DCB_I_Model(){
        //Model
        env = new GRBEnv();
        model = new GRBModel(env);
        model.set(GRB.StringAttr.ModelName, "SC_DCB_I model");
    }
    ...
    public void init_model( double alph,
        ...
        try{
            ...
            //Create decision variable configuration selection
            System.out.println("Creating decision variable u");
            u = new GRBVar[np][nc];
            for (int p = 0; p <np; p++){
                for (int c = 0; c < nc; c++){
                    u[p][c] = model.addVar(0,1,0,GRB.BINARY,
                        "u_p"+p+"_c"+c);
                }
            }
            ...
            for(int f=0; f<nf;f++){
                for(int k:Kf.get(f)){
                    ...
                    int j = Jfki[k][0];
                    for (int ti : Tfkj[k][j]){
                        obj.addTerm(alph*(T[ti-1]-r[k][j]),entry[k][j][ti-1]);
                    }
                    //cost of trajectory
            ...
            for (int p = 0; p <np; p++){
                for (int[] Sl_l:Sl){
                    for (int c : Sl_l){
                        //cost of having the configurations open
                        obj.addTerm(delt, u[p][c]);
                    }
                }
            }
            ...
            for (int l = 0 ;l<Sl.length;l++){
                for (int p= 0 ;p<np; p++){
                    ...
                    GRBLinExpr capacity_eff = GRBLinExpr();
                    capacity_eff.addConstant(M); //don't like doing it like this in case M is too big
                    for (int c :Sl[l]){
                        capacity_eff.addTerm(cap[l][p]-M,u[p][c]);//don't like doing it like this in case M is too big
                    }
                    model.addConstr(entry_count_sum,GRB.LESS_EQUAL,capacity_eff,
                         " Limit capacity of sector l"+l+ " during period p"+p);
                }
            }

            //Constraint "1 configuration per area control center per period"
            System.out.println("Creating contraints one configuration per area control center");
            for (int p = 0; p < np; p++){
                for(int[] ca : Ca){
                    GRBLinExpr Ilive_conf = new GRBLinExpr();
                    for(int conf :ca){
                        Ilive_conf.addTerm(1,u[p][conf]);
                    }
                    model.addConstr(Ilive_conf,GRB.EQUAL,1,"Only_one conf for ca "+ca);
                }
            }
    ...
    public boolean solve(   String log_file,
        ...
        try{
            ...
            if(model.get(GRB.IntAttr.Status) == GRB.Status.OPTIMAL) {
                ...
                for (int p = 0; p <np; p++){
                    for (int c = 0; c < nc; c++){
                        u_res[p][c]=u[p][c].get(GRB.DoubleAttr.X);
                    }
                }
            }
            else {
    ...
    public void print_solution( double alph,
        ...
        try {
            ...
            // USED CONFIGURATIONS
            System.out.println("Used configurations:");
            bw.write("Used configurations:\n");
            for (int p=0;p<np;p++){
                for (int c=0;c<nc;c++){
                    if (u_res[p][c]==1){
                        System.out.println("The configuration "+C[c]+" has to be opened from "+T[P[p][0]] + " to "+ T[P[p][1]]);
                        bw.write("The configuration "+C[c]+" has to be opened from "+T[P[p][0]] + " to "+ T[P[p][1]]+"\n");
                    }
                }
            }
